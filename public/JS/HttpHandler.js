class HttpHandler {
  static postData(url, callback) {
    if (!callback) return new Promise((resolve) => HttpHandler.post(url, resolve));
    return HttpHandler.post(url, callback);
  }
...
class Url {
  ...
  parametersToString() {
    let parsedParameter = '';
    for (const key in this.parameters) parsedParameter += `${key}=${this.parameters[key]}&`;
...
class FormElement {
  ...
  getInputValues() {
    const data = {};
    for (let i = 0; i < this.fields.length; i++) data[this.fields[i].name] = this.fields[i].value;
    return data;
  }
  ...
  onSubmitCallback(e) {
    if (this.settings.preventDefault && e != null) e.preventDefault();
    if (this.settings.body) return this.subCallback(this.getInputValues(), e);
...
class BarChart {
  ...
  getPosition(x, y) {
    ...
    if (this.alignment.x == this.directions.RIGHT) direction.x = -1;
    if (this.alignment.y == this.directions.BOTTOM) direction.y = -1;
  ...
  getTextPosition(text, x, y) {
    ...
    if (this.alignment.x == this.directions.RIGHT) position.x -= dimensions.width;
    return { text, width: dimensions.width, x: position.x, y: position.y };
  }
  ...
  getMinMax(data) {
    ...
    for (const key in data) {
      ...
      if (data[key] < min) min = data[key];
    }
    return { min, max };
  }

  renderBar(textPosition, data, minMax, maxLineHeight) {
    const barRealHeight = textPosition.y - 20 - maxLineHeight;
    const height = (data / minMax.max) * barRealHeight;
    this.ctx.fillRect(
      textPosition.x + this.ctx.measureText(textPosition.text).width / 2 - this.bar.width / 2,
      textPosition.y - 20,
      this.bar.width,
      -height
    );
  ...
  renderChart(data) {
    ...
    const paddingX = 50;
    const paddingY = 20;
    const maxWidth = this.canvas.width - paddingX;
    const maxHeight = this.canvas.height - paddingY;
    const scalerX = (maxWidth - paddingX - this.ctx.measureText('Tue').width) / 6;
    const scalerY = (maxHeight - paddingY * 3) / 3;
    this.ctx.beginPath();
    this.ctx.strokeStyle = 'rgba(32,6,59,1)';
    this.ctx.fillStyle = '#3e78b2';
    // this.ctx.fillRect(50,20,this.canvas.width-70,25+3*scalerY);
    const deltaMinMax = (minMax.max - minMax.min) / 3;
    this.ctx.strokeStyle = 'rgba(32,6,59,0.2)';
    for (let i = 0; i <= 3; i++) {
      const textPosition = this.getTextPosition(minMax.min + Math.floor(i * deltaMinMax), 20, 35 + i * scalerY);
      ...
      if (i == 3) maxLineHeight = textPosition.y - 6;
      // this.ctx.
    }
    for (let i = 0; i <= 6; i++) {
      const textPosition = this.getTextPosition(days[i], 40 + this.ctx.measureText(minMax.max).width + i * scalerX, 20);
...
class DOMCompiler {
  ...
  compileString() {
    ...
    this.domString.replace(this.regex, (g1, g2) => {
      if (this.variables[g2]) rawText = rawText.replace(g1, this.variables[g2].toString());
    });
    return rawText;
  }

  compileDOM() {
    const compiledText = this.compileString();
...
class AppendableElement {
  ...
  appendDom() {
    if (!this.isAppended) AppendableElement.appendElement.appendChild(this.element);
...
class FormPopup extends AppendableElement {
  ...
  copy(id) {
    ...
    const clonedForm = this.form.cloneNode(true);
  ...
  addColorElement(input) {
    const self = this;
    if (!this.colorPicker) this.colorPicker = new ColorPicker();
    input.addEventListener('focus', function () {
      ...
      input.addEventListener(
        'keyup',
        function (e) {
          if (e.key === 'Enter') return self.colorPicker.hide();
          if (input.value[0] == '#' && input.value.length >= 2) {
            if (!self.colorPicker.isActive) self.colorPicker.show();
  ...
  valueChange(input) {
    if (input.valueChangeCallback) input.valueChangeCallback(input);
  }
  ...
  setValue({ id = -1, value = 'NULL' }) {
    const field = this.getInputByName(id);
    if (!field) return;
    field.value = value;
  }
  ...
  removeElementWithAttribute(attr, value) {
    const children = this.form.childNodes;
  ...
  addButton({ type = 'submit', value = 'Save' }) {
    ...
    button.value = value;
    if (this.submit) this.removeElementWithAttribute('data-submit');
...
class ColorPicker extends AppendableElement {
  constructor(hex) {
    ...
    this.addColorBreakPoint({
      from: { value: 0, rgb: { r: 255, g: 0, b: 0 } },
      to: { value: 0.1666, rgb: { r: 255, g: 0, b: 255 } },
    });
    this.addColorBreakPoint({
      from: { value: 0.1666, rgb: { r: 255, g: 0, b: 255 } },
      to: { value: 0.3332, rgb: { r: 0, g: 0, b: 255 } },
    });
    this.addColorBreakPoint({
      from: { value: 0.3332, rgb: { r: 0, g: 0, b: 255 } },
      to: { value: 0.4998, rgb: { r: 0, g: 255, b: 255 } },
    });
    this.addColorBreakPoint({
      from: { value: 0.4998, rgb: { r: 0, g: 255, b: 255 } },
      to: { value: 0.6664, rgb: { r: 0, g: 255, b: 0 } },
    });
    this.addColorBreakPoint({
      from: { value: 0.6664, rgb: { r: 0, g: 255, b: 0 } },
      to: { value: 0.8333, rgb: { r: 255, g: 255, b: 0 } },
    });
    this.addColorBreakPoint({
      from: { value: 0.8333, rgb: { r: 255, g: 255, b: 0 } },
      to: { value: 1.0, rgb: { r: 255, g: 0, b: 0 } },
    });
  ...
  calculateColorBreakpointValue(position) {
    ...
    for (let i = 0; i < this.breakPoints.length; i++) {
      const point = this.breakPoints[i];
      if (this.isBetween(position, point)) {
        const normalize = (position - point.from.value) / (point.to.value - point.from.value);
        rgb = {
          r: Math.floor(point.from.rgb.r + normalize * (point.to.rgb.r - point.from.rgb.r)),
          g: Math.floor(point.from.rgb.g + normalize * (point.to.rgb.g - point.from.rgb.g)),
          b: Math.floor(point.from.rgb.b + normalize * (point.to.rgb.b - point.from.rgb.b)),
        };
        break;
      }
    }
  ...
  calculatePositionBreakpointValue(color) {
    for (let i = 0; i < this.breakPoints.length; i++) {
      ...
      if (affectedColor) {
        ...
        else delta = affectedColor.value / 255;
        const position = (point.from.value - delta * (point.to.value - point.from.value)) * this.slider.clientHeight;
        return position;
      }
    }
  }
  ...
  calculatePositionsFromRgb(rgb) {
    console.log('WEW', rgb);
    const minMax = this.getMinMax(rgb.r, rgb.g, rgb.b);
    this.ranger.thumb.position.y = minMax.max;
    this.ranger.thumb.position.x = minMax.max;
    const position = {};
    const normalizedPosition = {};
    const thumbHalf = this.ranger.thumb.clientWidth / 2;
    ...
    const flippedNormalizedPosition = { x: -normalizedPosition.x + 1, y: -normalizedPosition.y + 1 };
    const fnp = flippedNormalizedPosition;
    ...
    const sliderPosition = this.calculatePositionBreakpointValue(color);
    this.setThumbPosition(this.ranger, position);
    this.setThumbPosition(this.slider, { x: 0, y: sliderPosition - thumbHalf });
    if (rgb.r == rgb.g && rgb.g == rgb.b) color = { r: 255, g: 0, b: 0 };
  ...
  calculateColorRangerValue(ranger) {
    const sliderColor = this.slider.color;
    const position = { x: ranger.thumb.normalizedPosition.x, y: ranger.thumb.normalizedPosition.y };
  ...
  isBetweenColor(color, breakpoint) {
    if (color.r != 0 && color.r != 255) {
      if (
        breakpoint.from.rgb.g == breakpoint.to.rgb.g &&
        breakpoint.from.rgb.b == breakpoint.to.rgb.b &&
        color.b == breakpoint.from.rgb.b &&
        color.g == breakpoint.from.rgb.g
      )
        return { value: color.r, key: 'r' };
    } else if (color.g != 0 && color.g != 255) {
      if (
        breakpoint.from.rgb.r == breakpoint.to.rgb.r &&
        breakpoint.from.rgb.b == breakpoint.to.rgb.b &&
        color.r == breakpoint.from.rgb.r &&
        color.b == breakpoint.from.rgb.b
      )
  ...
  setGradient(color) {
    const grd = this.ranger.canvas.createLinearGradient(0, this.ranger.height, 0, 0);
    grd.addColorStop(0, 'black');
    grd.addColorStop(1, 'transparent');
    const grd2 = this.ranger.canvas.createLinearGradient(0, 0, this.ranger.width, 0);
  ...
  mouseSliderMove(element, e) {
    ...
    const thumbCenter = this.slider.thumb.clientHeight / 2;
    const yPos = (e.clientY - position.top - thumbCenter).clamp(-thumbCenter, this.slider.clientHeight - thumbCenter);
  ...
  mouseRangerMove(element, e) {
    const position = element.getBoundingClientRect();
    const thumbCenterY = this.ranger.thumb.clientHeight / 2;
    const thumbCenterX = this.ranger.thumb.clientWidth / 2;
    const yPos = (e.clientY - position.top - thumbCenterY).clamp(
      -thumbCenterY,
      this.ranger.clientHeight - thumbCenterY
    );
    const xPos = (e.clientX - position.left - thumbCenterX).clamp(
      -thumbCenterX,
      this.ranger.clientWidth - thumbCenterX
    );
    this.ranger.thumb.position = { x: xPos + thumbCenterX, y: yPos + thumbCenterY };
    this.ranger.thumb.normalizedPosition = {
      x: (xPos + thumbCenterX) / this.ranger.clientWidth,
      y: (yPos + thumbCenterY) / this.ranger.clientHeight,
    };
    this.color = this.calculateColorRangerValue(this.ranger);
  ...
  setListeners() {
    ...
    document.onmousemove = function (e) {
      if (self.slider.move) self.mouseSliderMove(self.slider, e);
      else if (self.ranger.move) self.mouseRangerMove(self.ranger.wrapper, e);
    };
    ...
    this.ranger.wrapper.addEventListener(
      'mousedown',
      function (e) {
...
String.prototype.repeatLastCharacter = function (length) {
  const times = length - this.length;
  if (times <= 0) return this;
  const string = this + this.slice(-1).repeat(times);
  return string;
};
