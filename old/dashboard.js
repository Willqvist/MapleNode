class DashbordFormElement {
  ...
  setValue(id, value) {
    const inps = this.form.getElementsByTagName('input');
    let input = null;
  ...
  addInput(settings) {
    ...
    const input = document.createElement(settings.type);
  ...
  build() {
    ...
    if (this.html) form.innerHTML += this.html;
    else {
      this.inputs.forEach((element) => {
        wrapperInput.appendChild(element);
      });
      wrapper.appendChild(wrapperInput);
      if (this.afterHtml) wrapper.innerHTML += this.afterHtml;
    }
    ...
    const pickers = this.form.getElementsByTagName('input');
    for (let i = 0; i < pickers.length - 1; i++) {
      const picker = pickers[i];
      if (picker.className.includes('colorPickerInput')) this.addColorElement(picker);
    }
  }
...
class Panel {
  ...
  setMenuItem(item) {
    ...
    this.menuIcon = { icon, class: icon.className };
  }
  ...
  show() {
    ...
    this.onShow(() => {
      if (this.loadable) this.menuIcon.icon.className = this.menuIcon.class;
  ...
  onClick(btn, callback) {
    if (!this.elements[btn]) this.elements[btn] = this.element.getElementsByClassName(btn)[0];
  ...
  onClassClick(cls, callback) {
    const elems = this.element.getElementsByClassName(cls);
...
class AddPanel extends Panel {
  ...
  init() {
    const holder = this.contentHolder;
    this.addForm.onSubmit((data) => {
      const url = new Url(`./dashboard/${this.url}/add/`, {
        name: data.Name,
        url: data.Url,
        nx: data.Nx,
        time: data.Time,
      ...
      HttpHandler.postData(url, (response) => {
        if (response.success) {
          ...
          const download = element.getElementsByClassName('downloadBox')[0];
          download.setAttribute('data-id', response.id);
          const clickElement = download.getElementsByClassName('edit')[0];
          const removeElement = download.getElementsByClassName('remove')[0];
    ...
    this.deleteForm.onSubmit((data) => {
      const url = new Url(`./dashboard/${this.url}/remove/`, { id: data.attributes.id });
      HttpHandler.postData(url, (response) => {
...
class DownloadPanel extends AddPanel {
  ...
  init() {
    ...
    this.editForm.onSubmit((data) => {
      const url = new Url('dashboard/download/update', { name: data.Name, url: data.Url, key: data.attributes.key });
      HttpHandler.postData(url, (response) => {
        const parent = document.getElementById(`download-key-${data.attributes.key}`);
...
class VotePanel extends AddPanel {
  ...
  init() {
    super.init();
    this.editForm.onSubmit((data) => {
      const url = new Url('dashboard/votes/update', {
        name: data.Name,
        url: data.Url,
        nx: data.Nx,
        time: data.Time,
      ...
      console.log(data);
      HttpHandler.postData(url, (response) => {
        const parent = document.getElementById(`vote-key-${data.attributes.key}`);
        this.setValue(parent, 'Name', data.Name, this.setInnerHTML);
        this.setValue(parent, 'Url', data.Url, this.setInnerHTML);
...
class DesignPanel extends Panel {
  constructor(name, currentHeroImage) {
    ...
    const url = new Url('./dashboard/changeImage');
    ...
    HttpHandler.postData(url, (response) => {
      if (response.success) {
        ...
        this.editHero.build();
        const editHeroTitle = this.editHero.form.getElementsByTagName('h3')[0];
        ...
        const { form } = this.editHero;
        const fileElements = form.getElementsByClassName('heroImageEdit');
        for (let i = 0; i < fileElements.length; i++) {
          const image = fileElements[i];
          image.addEventListener(
            'click',
            () => {
              // NEW IMAGE SELECTED
              if (this.editHero.attributes.hero) {
                const url = new Url('./dashboard/heroImage/change', { file: image.getAttribute('data-image') });
                HttpHandler.postData(url, (response) => {
                ...
                const url = new Url('./dashboard/logo/change', { file: image.getAttribute('data-image') });
                HttpHandler.postData(url, (response) => {
                  if (!response.success) console.log(response.reason);
                  this.editHero.hide();
                  for (let j = 0; j < fileElements.length; j++) {
  ...
  init() {
    const display = this.editPalette.form.getElementsByClassName('paletteDisplay')[0];
    const mainColor = display.getElementsByClassName('mainColor');
    const secondaryMainColor = display.getElementsByClassName('secondaryMainColor');
    const fontColorDark = display.getElementsByClassName('fontColorDark');
    const fontColorLight = display.getElementsByClassName('fontColorLight');
    const fillColor = display.getElementsByClassName('fillColor');
    this.editPalette.onChange((input) => {
      console.log('changed!');
      switch (input.name) {
        case 'mainColor':
          this.forEachClass(mainColor, (e) => {
            e.style.background = input.value;
          });
          break;
        case 'secondaryMainColor':
          this.forEachClass(secondaryMainColor, (e) => {
            e.style.background = input.value;
          });
          break;
        case 'fontColorDark':
          this.forEachClass(fontColorDark, (e) => {
            e.style.color = input.value;
          });
          break;
        case 'fontColorLight':
          this.forEachClass(fontColorLight, (e) => {
            e.style.color = input.value;
          });
          break;
        case 'fillColor':
          this.forEachClass(fillColor, (e) => {
            e.style.background = input.value;
          });
          break;
      }
    ...
    this.editPalette.onSubmit((data) => {
      ...
      const { add } = data.attributes;

      if (add) {
        delete data.attributes;
        const url = new Url('dashboard/palette/add', data);
        HttpHandler.postData(url, (response) => {
          if (response.success) {
            ...
            div.innerHTML = `
                        ...
                        `;
            const select = div.getElementsByClassName('select')[0];
            select.addEventListener(
              'click',
              this.onSelectClick.bind({ elem: select, select: this.select, onSelect: this.onSelectClick }),
              false
            );
            const elem = div.getElementsByClassName('downloadBox')[0];
            console.log(elem);
            get('boxFlexHolder', get('designpanel'))[0].appendChild(elem);
            elem.getElementsByClassName('editPalette')[0].addEventListener(
              'click',
              (e) => {
            ...
            elem.getElementsByClassName('remove')[0].addEventListener(
              'click',
              (e) => {
        ...
        const url = new Url('dashboard/palette/update', data);
        HttpHandler.postData(url, (response) => {
          if (response.success) {
            const element = data.attributes.element.parentNode.parentNode;
    ...
    this.removePalette.onSubmit((data) => {
      ...
      const url = new Url('dashboard/palette/remove', { key });
      HttpHandler.postData(url, (response) => {
        if (response.success) {
    ...
    this.onSelectClick = function () {
      if (this.elem && !this.elem.className.includes('activeBtn')) this.select(this.elem, this.onSelect, this.select);
    };
    this.forEachElement('select', (elem) => {
      elem.addEventListener(
        'click',
        this.onSelectClick.bind({ elem, select: this.select, onSelect: this.onSelectClick }),
        false
      );
    });
    this.forEachElement('remove', (elem) => {
  ...
  select(elem, onSelect, select) {
    console.log(elem);
    const url = new Url('dashboard/palette/select', { key: elem.getAttribute('data-key') });
    HttpHandler.postData(url, (response) => {
      if (response) {
        const element = get('activeBtn')[0];
        if (element) {
          ...
          element.addEventListener('click', this.onSelect.bind({ elem: element, select, onSelect }), false);
        }
        elem.className += ' activeBtn';
        elem.innerHTML = `<i class="fas fa-check"></i>`;
        elem.setAttribute('data-info', 'Current Palette');
        elem.removeEventListener('click', this.onSelect.bind({ elem, select, onSelect }), false);
      }
    });
  }
}
...
class LayoutPanel extends Panel {
  ...
  setZindex(box) {
    if (this.lastHover) this.lastHover.style.zIndex = '10';
  ...
  init() {
    ...
    height.value = 120;
    const rows = document.getElementById('aor');
    rows.value = 13;
    const columns = document.getElementById('aoc');
    columns.value = 9;
    ...
    const layoutMover = new Mover('layoutMover', Direction.Y);
    ...
    this.editPanelForm.onSubmit((data) => {
      ...
      const element = this.container.getElementsByClassName(`panel_id_${data.attributes.panel}`)[0];
    ...
    this.forEachElement('layout_box', (box) => {
      box.mouseDown = false;
      const panel = this.panels[box.getAttribute('data-panelID')];
      const mover = new Mover();

      mover.setMoverElement(box.getElementsByClassName('mover')[0], Direction.Y);

      const horizontalMover = new Mover();
      ...
      if (panel) {
        ...
        horizontalMover.onMove((delta) => {
          const size = panel.columns.size + Math.floor((-delta / this.pixelSize) * columns.value);
        ...
        mover.onMove((delta) => {
          const size = panel.rows.size + Math.floor((-delta / this.pixelSize) * rows.value);
        ...
        document.addEventListener(
          ...
          (e) => {
            if (box.mouseDown) {
              const deltaPos = { x: e.clientX - box.mousePos.x, y: e.clientY - box.mousePos.y };
              const normalizedPos = {
                x: Math.floor((deltaPos.x / this.pixelSize) * columns.value),
                y: Math.floor((deltaPos.y / this.pixelSize) * rows.value),
              };

              console.log(normalizedPos.y + panel.newSize.rowSize);

              if (
                panel.rows.pos + normalizedPos.y > 0 &&
                normalizedPos.y + panel.rows.size <= parseInt(rows.value) + 1
              ) {
                box.style.gridRow = `${Math.max(1, panel.rows.pos + normalizedPos.y)}/${
                  normalizedPos.y + panel.rows.size
                }`;
                panel.newPos.y = normalizedPos.y;
              }
              if (
                panel.columns.pos + normalizedPos.x > 0 &&
                normalizedPos.x + panel.columns.size <= parseInt(columns.value) + 1
              ) {
                box.style.gridColumn = `${Math.max(1, panel.columns.pos + normalizedPos.x)}/${
                  normalizedPos.x + panel.columns.size
                }`;
        ...
        this.saveBtn.addEventListener(
          ...
          () => {
            if (this.edited) {
              ...
              const url = new Url('/dashboard/layout', { json: JSON.stringify(data), name: 'home' });
              HttpHandler.postData(url, (response) => {});
            }
          },
          false
      ...
      HttpHandler.getData(url, (resp) => {
        if (resp.success) {
          ...
          Object.keys(this.panels).forEach((key) => {
            ...
            const box = div.getElementsByClassName('layout_box')[0];
...
function get(name, parent = document) {
  ...
  if (!id) id = parent.getElementsByClassName(name);
